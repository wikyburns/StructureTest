...
@warning When creating a new application implementing a sensor controller delegate, the main `Info.plist` needs to
...
used to set both exposure and whitebalance to auto.
...
@param captureSession The capture session object that signaled the delegate.
@param sample An NSDictionary containing the sample objects (e.g. STDepthFrame, STColorFrame, etc) specified by the
sample type.
...
/** Creates a new implementation of STCaptureSession - refined and optimized. */
+ (instancetype)newCaptureSessionBeta;
...
- `kSTCaptureSessionPropertySensorIRProjectorModeKey` :
  - Specifies whether the IR projector shall be turned ON or OFF (Mark II sensors only, currently).
  - `BOOL` value equal.
  - Defaults to YES.
