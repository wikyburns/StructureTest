...
@param glProjection Projection matrix used during rendering. See also [STDepthFrame glProjectionMatrix] and
[STColorFrame glProjectionMatrix].
@param alpha A float value for transparency between 0 (fully transparent) and 1 (fully opaque)
@param highlightOutOfRangeDepth Whether the areas of the mesh which are below Structure Sensor minimal depth range
should be highlighted in red.
@param wireframe Whether to render a wireframe of the mesh or filled polygons. When enabled, STMapper needs to be
initialized with the `kSTMapperEnableLiveWireFrameKey` option set to `@YES`.

@return TRUE on success, FALSE if there is no STMapper attached to the same STScene with the corresponding (wireframe or
triangles) live mode enabled.
*/
...
@param cameraModelView A GLKMatrix4 camera position used for rendering.
@param glProjection Projection matrix used during rendering. See also [STDepthFrame glProjectionMatrix] and
[STColorFrame glProjectionMatrix].
@param alpha A float value for transparency between 0 (fully transparent) and 1 (fully opaque)
@param highlightOutOfRangeDepth Whether the areas of the mesh which are below Structure Sensor minimal depth range
should be highlighted in red.
@param minDepthToHighlightInMeters Specifies minimal depth range to be highlighted in red.
@param wireframe Whether to render a wireframe of the mesh or filled polygons. When enabled, STMapper needs to be
initialized with the `kSTMapperEnableLiveWireFrameKey` option set to `@YES`.
