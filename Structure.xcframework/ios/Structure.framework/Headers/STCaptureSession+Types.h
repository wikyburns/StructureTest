...
typedef NS_ENUM(NSInteger, STCaptureSessionIOSCamera) {
    ...
    /// Indicates the front-facing, fixed-focus iOS color camera and True Depth camera (on supported devices) are used.
...
typedef NS_OPTIONS(NSUInteger, STCaptureSessionUserInstruction) {
    ...
    */
    ...
    /// Indicates that the offline operational period for 3d-party sensors is finished
    /// and internet connection is required to unlock the SDK
    STCaptureSessionUserInstructionNeedInternetConnection = 1 << 8,
};
