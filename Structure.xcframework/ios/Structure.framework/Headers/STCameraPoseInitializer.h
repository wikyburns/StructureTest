...
@param volumeSize The current volume size in meters. Not used by the
...
@param depthFrame the current processed depth from Structure Sensor. Can be `nil` if using the strategies
...
@param outputMask should point to an allocated buffer of (`depthFrame.width * depthFrame.height`) pixels. A mask pixel
will be 255 if the corresponding 3D point fits inside the volume, 0 if outside or if there is no depth.
*/
